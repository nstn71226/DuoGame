...
export let flagCloth = null; // Biến để đổi màu cờ
...
const SAVE_POINT_2 = new THREE.Vector3(490, 1, 472); // Tọa độ điểm lưu thứ 2
...
// Lưu lại người đang lái pháo để chống tranh giành
let activeCannonPlayer = null; 
// Cờ báo hiệu nhả súng để gửi mạng
...
        // -------------------------------------------------------------
        // 🔑 TẠO VẬT LIỆU MÀU VÀNG GOLD VÀ GÁN CHO CHÌA KHÓA
        // -------------------------------------------------------------
...
    // === 2. TẢI ZOMBIE VÀ ĐỒNG BỘ ANIMATION ---
...
    // === 4. GỌI BOSS XUẤT HIỆN Ở TỌA ĐỘ MỚI (490, 1, 450) ===
    finalBoss = new Boss(scene, new THREE.Vector3(490, 1, 450), colliders);

    // =============================================================
    // 🚩 TẠO LÁ CỜ CHO SAVE POINT 2
    // =============================================================
...
    pole.position.y = 1; // Nâng cột lên
    pole.castShadow = true;
    flagGroup.add(pole);

    // 2. Tạo vải cờ (Lúc đầu màu đỏ)
...
    flagCloth.position.set(1, 2.5, 0); // Đặt cờ dính vào góc trên của cột
...
// 💡 HÀM NHẢ CHÌA KHÓA KHI CHẾT
function resetKeys() {
    if (button1) {
        button1.userData.isCollected = false;
        button1.position.set(475, 0.5, 520);
        if (!button1.parent) window.sceneReferenceMap4.add(button1); 
    }
    if (button2) {
        button2.userData.isCollected = false;
        button2.position.set(448, 0.5, 466);
        if (!button2.parent) window.sceneReferenceMap4.add(button2);
    }
}

// === 6. HÀM LOGIC CẬP NHẬT CHÍNH ===
export function updateMap4(player1, player2, delta = 0.016, scene) {
    if (!barrier || !player1.object || !player2.object) return {};
    
    // Lưu tạm scene vào window để hàm resetKeys dùng được
    window.sceneReferenceMap4 = scene;
    
    cannonJustReleased = false; // Reset cờ mạng mỗi frame
...
    // ----------------------------------------
    // 🔥 LOGIC BÃI LỬA (VÀ RƠI CHÌA KHÓA QUAY LẠI)
    // ----------------------------------------
    if (fireZone) {
...
            if (zombieData.model && !zombieData.isDead) {
...
  if (finalBoss) {
        ...
        // 💡 Bật cơ chế vòng tròn đánh boss ở map 4
