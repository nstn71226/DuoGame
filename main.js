...
bgMusic.loop = true;
bgMusic.volume = 0.15; // Chỉnh nhỏ nhẹ nhàng để k bị điếc tai
...
// 💡 THÊM MỚI: ÂM THANH CLICK MENU
const clickSound = new Audio('models/amthanhclick.mp3');
clickSound.volume = 1.0; // Vặn max volume cho tiếng click rõ ràng

// Bắt sự kiện click trên toàn bộ trình duyệt
document.addEventListener('click', (e) => {
    // Nếu click trúng thẻ <button> hoặc dấu X tắt bảng (class close-modal)
    if (e.target.tagName === 'BUTTON' || e.target.closest('button') || e.target.classList.contains('close-modal')) {
        // Kiểm tra xem người chơi có đang tắt SFX trong cài đặt không
        if (window.isSFXOn !== false) {
            clickSound.currentTime = 0; // Trả về 0 để lỡ bấm spam 2 lần liên tục vẫn kêu đủ 2 tiếng
...
        4: [new THREE.Vector3(490, 8, 490), new THREE.Vector3(491, 8, 490)]
    };
    
...
      // Tìm điểm chính giữa của 2 người chơi
...
      // Di chuyển vùng nhận bóng (Target) tới trung điểm
      lightTarget.position.set(midX, midY, midZ);
      
      // Di chuyển nguồn sáng (Directional Light) theo Target giữ nguyên góc chiếu 
      // Góc lệch chuẩn: X + 20, Y + 35, Z + 15
      dirLight.position.set(midX + 20, midY + 35, midZ + 15);
      
      // Cập nhật lại khung hắt bóng để lưới bóng chạy theo mượt mà
      dirLight.shadow.camera.updateProjectionMatrix();
  }
